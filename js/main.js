  ...
  methods:{
    ...
    endGame(){
      ...
      //изначальный
          ...
          this.score.primal >this.score.terran && this.score.primal == 8) {
        this.goToResult('promal');
        this.totalGame.primal++;
      }
      //протос
      else if (this.score.protoss > this.score.zerg && 
            this.score.protoss >this.score.terran && this.score.taldarim < 5 && 
            Math.abs(this.score.protoss - this.score.zerg) > 3) {
        this.goToResult('protoss');
        this.totalGame.protoss++;
      }
       //талдарим
       else if (this.score.taldarim > this.score.zerg && 
          this.score.taldarim >this.score.terran && this.score.taldarim == 5) {
...
let audionBtn= document.querySelector('.btn__sound');
let audioIcn = document.querySelector('.btn__sound i');

audio.addEventListener('loadmetadata', function(){
  audio.currentTime = 0 + Math.random() * (audio.duration +  1 - 0)
})

audionBtn.addEventListener('click', function() {
  if (audio.muted) {
    audio.muted = false
    audioIcn.classList.add('fa-volume-up')
    audioIcn.classList.remove('fa-volume-off')
  } else if (!audio.muted) {
    audio.muted = true
    audioIcn.classList.add('fa-volume-off')
    audioIcn.classList.remove('fa-volume-up')
  }
})
